...
function renderObituarySizeOptions(){
  ...
  if(!selectedCity || !selectedPub || !content){
    box.innerHTML = '<div class="empty">Select City/Publication and type content to see size options.</div>';
    selectedSize = null;
    checkReady();
    return;
  }
  ...
  box.innerHTML = tiers.map((t, i) => {
    ...
    return `<div class="size-opt" data-idx="${i}">
...
function getDefaultHeading(category){
  ...
  if(category === 'Obituary') return 'IN LOVING MEMORY';
...
function buildArtwork(category, content, widthCm, heightCm, headingTextOverride){
  ...
  const dateVal = document.getElementById('adDate') ? document.getElementById('adDate').value : '';
  ...
  let inner;
  if(isObituary){
    // Placeholder layout until real sample ads are provided — photo box, centered
    // heading, memorial text, and the date if one was entered.
    inner = `
      <div class="tmpl-obituary">
        ...
        <div class="obituary-photo" contenteditable="false">Photo</div>
