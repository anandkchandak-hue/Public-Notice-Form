...
let selectedGalleryDesign = 'design-cream-memorial'; // default gallery style
...
function openDesignGallery(){
  ...
  overlay.querySelectorAll('.design-gallery-item').forEach(item=>{
    item.onclick = ()=>{
  ...
  document.getElementById('designGalleryCloseBtn').onclick = ()=>{
    document.body.removeChild(overlay);
  };
...
document.getElementById('catSelect').addEventListener('change', (e)=>{
  ...
  document.getElementById('translateRow').style.display = isObituary ? 'none' : '';
  if(isObituary){
    obituaryDesignImageDataUrl = null;
    renderObituarySizeOptions();
  } else {
...
function clearForm(){
  ...
  document.getElementById('translateRow').style.display='';
