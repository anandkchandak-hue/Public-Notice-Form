...
document.getElementById('imgFileInput').addEventListener('change', async (e)=>{
  ...
  try{
    ...
    if(text){
      ...
      statusEl.innerText = 'Text extracted from image — please check it for accuracy before generating the ad.';
      suggestSize();
      recalc();
    } else {
      statusEl.className = 'status err';
      statusEl.innerText = 'Could not find readable text in that image — try typing it manually, or check the language selected matches the image.';
    }
  }catch(err){
    statusEl.className = 'status err';
    statusEl.innerText = 'Could not read the image — try typing the content manually.';
  }
  e.target.value = '';
});
