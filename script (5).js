...
function buildArtworkFromRealTemplate(template, content, widthCm, heightCm){
  ...
  // Solid background colours matching the template's own design — these fully cover the
  // original sample photo/text underneath, so the customer's real content looks like it
  // was always part of the design instead of being pasted on top of the sample.
  const photoZoneBg = template.photoZoneBg || '#ffffff';
  const textZoneBg = template.textZoneBg || '#ffffff';
  ...
  return `
    <div class="artwork-scale-wrap">
      <div class="artwork-canvas" id="artworkCanvas" style="${canvasStyle}">
        ...
        <div style="position:absolute; left:${pz.left}%; top:${pz.top}%; width:${pz.width}%; height:${pz.height}%; overflow:hidden; z-index:1; background:${photoZoneBg};">
          ${photoInner}
        </div>
        <div id="artworkBodyText" contenteditable="true" data-base-size="${sizePt}"
          style="position:absolute; left:${tz.left}%; top:${tz.top}%; width:${tz.width}%; height:${tz.height}%; overflow:hidden; z-index:1; text-align:${align}; font-size:${sizePt}pt; font-family:${font}; color:${color}; background:${textZoneBg}; padding:2px;">${safeContent}</div>
      </div>
    </div>
